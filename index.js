...
const scrapperPuppeteer_1 = __importDefault(require("./scrapperPuppeteer"));
...
(async () => {
    ...
    if (response.value === 'screenshot') {
        scrapperPuppeteer_1.default.screenshot(response.url, response.fileName);
    }
    if (response.value === 'wikiSummary') {
        const htmlWikiPageSummary = await scrapperPuppeteer_1.default.extractHtmlWikiPageSummary(response.url);
